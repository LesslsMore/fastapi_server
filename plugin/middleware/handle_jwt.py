...
class AuthTokenMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if not auth_token:
            return JSONResponse(status_code=HTTP_401_UNAUTHORIZED, content={"code": 200, "msg": "用户未授权,请先登录", "data": None})
        uc, err = parse_token(auth_token)
        if uc is None:
            return JSONResponse(status_code=HTTP_401_UNAUTHORIZED, content={"code": 200, "msg": "身份验证信息异常,请重新登录!!!", "data": None})
        t = await get_user_token_by_id(uc.user_id)
        if not t:
            return JSONResponse(status_code=HTTP_401_UNAUTHORIZED, content={"code": 200, "msg": "身份验证信息已失效,请重新登录!!!", "data": None})
        if t != auth_token:
            return JSONResponse(status_code=HTTP_401_UNAUTHORIZED, content={"code": 200, "msg": "账号在其它设备登录,身份验证信息失效,请重新登录!!!", "data": None})
